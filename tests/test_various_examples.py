...
`approximate_q_range` module in order to perform various examples of
q-numerical range and q-numerical radius approximations.
"""
...
# The test case arguments are imported from the auxiliary
# `test_various_examples_arguments` Python module.
from test_various_examples_arguments import TEST_ARGUMENTS
...
OUTPUT_PATH = r"../test_various_examples_output"
...
def save_comparison_figure(
    ...
    """
    This function accepts two approximated q-numerical ranges and creates a
    figure where both of them are displayed one atop of the other via the
    functionalities from the `matplotlib.pyplot` package. Both ranges are
    displayed in such a way that only their boundary is colored, while the
    interior is not. The boundary of the first q-numerical range is colored in
    red, while the boundary of the second is colored in blue. The created
    figure is subsequently saved in the form of an image file.

    :arg q_range_1: The first input q-numerical range, given in the form of a
        `scipy.spatial.ConvexHull` or `np.ndarray` object.
    :arg q_range_2: The second input q-numerical range, given in the form of a
        `scipy.spatial.ConvexHull` or `np.ndarray` object.
    ...
    if isinstance(q_range_1, ConvexHull):
        ...
        # `ConvexHull` object, then the points of this convex hull should be
        # selected and used in order to draw an approximative polygon.
        plt.fill(
        ...
        # However, if the first q-numerical range is represented in the form of
        # an `np.ndarray` object, then all the corresponding points should just
        # be directly drawn as they are.
        plt.plot(
            q_range_1[:, 0],
            q_range_1[:, 1],
            color="r",
    ...
    # The second q-numerical range should be handled in an identical manner.
...
def execute_test_case(
    ...
    """
    This function executes a provided test case. More precisely, it takes an
    input matrix and q-value, and then approximately determines the
    corresponding q-numerical range and q-numerical radius of the given matrix.
    The provided number of feasible vector selections is performed and the
    approximation is done twice: with and without the help of Lemma 3.1. The
    obtained results are saved in two files:
    1. A text file which contains the approximated q-numerical radius, with and
    without the help of Lemma 3.1.
    ...
    q-numerical range approximated via Lemma 3.1. Both of these files are named
    after two corresponding identifiers, the first of which denotes the test
    case dictionary index, while the second denotes the selected q-value index.
    ...
    :arg output_folder: A `Path` object that signifies the folder where the two
        output files should be saved.
    :arg test_id: A nonnegative integer which represents the test case group
        identifier, i.e., the index of the test case dictionary that is being
        used.
    :arg q_value_id: A nonnegative integer that signifies the index of the
        selected q-value.
    """

    # Convert the `test_id` and `q_value_id` identifiers into a string with
    # precisely four characters. For example, 13 should get transformed to
    # "0013".
    test_id_string = str(test_id).zfill(4)
    q_value_id_string = str(q_value_id).zfill(4)

    # Determine the names of the two output files.
    ...
    # Save both approximated q-numerical radii to the output text file.
    with open(str(output_folder / output_log_name), "w") as opened_file:
        opened_file.write(
            f"Approximation process done via the naive selection mechanism. "
            f"The approximated q-numerical radius is {q_radius_1:.4f}.\n"
        )
        opened_file.write(
    ...
    # Use the auxiliary `save_comparison_figure` function in order to create
    # the desired q-numerical range graphical representation and then save it
...
if __name__ == "__main__":
    ...
    for test_id, test_arguments in enumerate(TEST_ARGUMENTS):
        # Iterate through all the possible q-values corresponding to the given
        # input matrix...
